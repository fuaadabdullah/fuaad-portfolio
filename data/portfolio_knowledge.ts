export const siteFacts = `
This is the portfolio of Fuaad — finance major, full-stack developer.
Built with Next.js 13+, App Router, TypeScript, Tailwind CSS, and MDX blogs.
Projects include RIZZK Calculator, personal portfolio site, and various tools.

He offers services:
– Web app builds
– MVP tooling
– Custom dashboards

Blog topics cover:
– Next.js performance
– Portfolio development
– Trade tools and coding insights.
`;

export const faq = [
  {
    trigger: ["services", "offer", "what do you do"],
    answer: "Fuaad offers web app builds, MVP tooling, custom dashboards, and developer utilities."
  },
  {
    trigger: ["tech stack", "built with", "tech"],
    answer: "This portfolio site is built using Next.js App Router, TypeScript, MDX for blogs, Tailwind CSS, Framer Motion, and custom tooling."
  },
  {
    trigger: ["contact", "reach you", "email"],
    answer: "You can contact Fuaad through the contact form or via email at contact@heyimfuaad.me."
  }
];